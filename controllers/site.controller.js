...
exports.create = (req, res) => {
  // Validate request
  if (!req.body.content) {
    return res.status(400).send({message: "Site content cannot be empty"});
  }

  // Create a Site
  const site = new Site({content: req.body.content});
...
exports.update = (req, res) => {
  //Validate Request
  if (!req.body.content) {
    return res.status(400).send({message: "Site content can not be empty"});
  }
...
exports.delete = (req, res) => {
  Site.findByIdAndRemove(req.params.siteId).then(site => {
    if (!site) {
      return res.status(404).send({
        message: "Site not found with id " + req.params.siteId
      });
