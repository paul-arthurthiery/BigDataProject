...
exports.create = (req, res) => {
  // Validate request
  if (!req.body.content) {
    return res.status(400).send({message: "Consumption content cannot be empty"});
  }

  // Create a Consumption
  const consumption = new Consumption({content: req.body.content});
...
exports.findAll = (req, res) => {
  Consumption.find().then(consumptions => {
...
exports.findOne = (req, res) => {
  Consumption.findById(req.params.consumptionId).then(consumption => {
    if (!note) {
...
exports.update = (req, res) => {
  //Validate Request
  if (!req.body.content) {
    return res.status(400).send({message: "Consumption content can not be empty"});
  }
...
exports.delete = (req, res) => {
  Consumption.findByIdAndRemove(req.params.consumptionId).then(consumption => {
    if (!consumption) {
      return res.status(404).send({
        message: "Consumption not found with id " + req.params.consumptionId
      });
